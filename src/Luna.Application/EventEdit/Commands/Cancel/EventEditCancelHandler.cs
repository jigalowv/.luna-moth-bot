...
public class EventEditCancelHandler 
    : IRequestHandler<EventEditCancelRequest, ErrorOr<string?>>
{
    private readonly IExecutorRepository _executorRepository;
    private readonly IEventEditRepository _eventEditRepository;

    public EventEditCancelHandler(
    ...
    public async Task<ErrorOr<string?>> Handle(
        EventEditCancelRequest request, 
        CancellationToken ct)
    {
        ...
        bool success = await _eventEditRepository
            .CancelAsync(eventEdit.EventId, ct);
